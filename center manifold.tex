...
That is we prove the existence and smoothness of the center manifolds for  stochastic evolution equation~\eqref{eq(2.1)}. The results generalize the work of  Duan  et al.~\cite{Duan1, Duan2}, who proved the existence and smoothness of center-stable and center-unstable  manifolds.
...
\begin{definition} \label{trich}A
...
Example~\ref{sec:sdwe} establishes that a semilinear damped wave equation has a center manifold.  Lastly, Example~\ref{sec:idcm} explores a case where the center manifold itself is infinite dimensional.
...
\end{equation}
We set the space $H:=L^2(0,\pi)$, the operator $Au= {d^2u}/{dx^2}+u$ with domain $D(A):=H^2(0,\pi)\cap H_0^1(0,\pi)$.  Then
the spectrum of the operator~$A$ is
\begin{equation*}\sigma(A)=\{1-n^2\mid n=1,2,\ldots\}. \end{equation*}
The corresponding complete and orthogonal  eigenfunctions  are~$\sin nx$\,, $n=1,2,\ldots$\,. The operator~$A$  satisfies the exponential trichotomy~\ref{??}: $H^c=\operatorname{span}\{\sin x\}$, $H^s=\operatorname{span}\{\sin nx, n=2,3,\ldots\}$,  and $H^u=\emptyset$.


The random variable $z(\theta_t\omega)$ is the stationary solution of  the Ornstein--Uhlenbeck \sde~(\ref{eq(2.4)}). In order to transform to random parabolic equation, let the transform $u=u^*\exp(\sigma z(\theta_t\omega))$. %\marginpar{??from (38)?? you must cross-reference explicitly}
...
Select parameter~$a$ small enough such the conditions of  Theorem~\ref{Thm(3.1)} is satisfied.  Then there exists %\marginpar{??clarify "we have": does it mean "exists"? or something else? does theorem~5 justify the statement? or not?  clarify. In general, do not use "we have"}
a slow (center) manifold
...
From equation \eqref{th2}, the random dynamical system on the slow manifold~$M^c(\omega)$ satisfies%\marginpar{??justify, cross-reference explicitly??}
...
\end{eqnarray}
Integrating against $\sin x$, it follows that%\marginpar{??justify??}
\begin{eqnarray*}
\frac {ds}{dt}=s\sigma z(\theta_t\omega)-\frac 3 4 {\exp[2\sigma z(\theta_t\omega)]}  as^3+\Ord{s^5}.
\end{eqnarray*}
%So $u=0$ is stable if $a<0$.
We now  calculate the slow manifold~$h^c(s\sin x,\omega)$. For $\omega\in \Omega$, $c_n(s,\omega)$ is odd functions of  $s$ since $-u$ is a solution of equation \eqref{pa1}. Assume % \marginpar{??what evidence is there that this form is good??}
...
Note that $z(\theta_t\omega)=-\int_{-\infty}^0\exp(\tau)\omega(\tau+t)\,d\tau+\omega(t)=\int_{-\infty}^t\exp(s-t)dW(s)$.  In fact this is the convolution term~\cite{Rob1}. From the Taylor series and $E(z(\theta_t\omega))=0$, the expectation of stochastic centre manifold
is
%\marginpar{??Do not use double dollars---they are long outdated.??}
%marginpar{??Further, I do not see how it can be correct. What is the justification?? how can you take the expectation inside the exp and the reciprocal??}
...
%\marginpar{Must relate to what my analysis gives??  Need to compare and contrast.  Which here involves rewriting the computer algebra in sensible forms: we cannot have this verbatim output in the document??  Interpret and incorporate appropriately.}
...
Denote the noise $\phi(0,\, \{k\})=\int_{-\infty}^t\exp[(-k^2-1)(t-s)]dW(s)$, then
\begin{eqnarray}
...
Wang and Duan~\cite{ Wang}  discussed a semilinear wave equation with a two dimensional center-unstable manifold.  We establish that the   semilinear damped wave equation has a one dimensional center manifold.    %For the deterministic case see  Carr's book~\cite[Example 6.4]{Carr}.
...
$W(t)$~is the standard $\mathbb{R}$-valued Wiener process on a probability space~$(\Omega,\mathcal{F}, \mathbb{P} )$. The nonlinearity term $f$ is smooth and $f(0)=0$. 


Let the space ${H}=H_0^1(0,2\pi)\times L^2(0,2\pi)$. Rewrite the
...
$z(\omega)$~is the stationary solution of~(\ref{eq(2.4)}). Then equations \eqref{shyp1} and \eqref{shyp2} are transformed to
...
with negative real part. By using a new inner product~\cite{Wang},  we  define an equivalent norm on~${H}$ with the eigenvectors $\{(1, \delta_k^\pm)\sin kx \}$ orthogonal in this norm.
...
Let $\Psi=rq_1^++sq_2^++y$\,, $y\in P^s H$\,.  Projecting the random system~\eqref{RHPY} to the center subspace span$\{q^+_2\}$ by the projection $P^c$, %\marginpar{why?? justify??}
\begin{eqnarray}
\dot{s}q_2^+=Z(\theta_t\omega)sq_2^++P^cF(\theta_t\omega, rq_1^++sq_2^++y).
\end{eqnarray}
%\marginpar{??why does it apply??}
By a cut-off function,  $\Lip  F$ is sufficient small. From Theorem~\ref{Thm(3.1)} we obtain a center manifold~$M(\omega)$ which we denote as the graph of a
...
There are very few rigorous results involving infinite dimensional center manifolds except the existence theorem of Gallay~\cite{Gal} and the cylindrical analysis of Mielke~\cite{Mie}.  However, applied mathematicians and engineers implicitly, in effect, assume an infinite dimensional center manifold whenever they invoke the method of multiple scales for slowly varying solutions~\cite[e.g.]{Roberts88a, Roberts92c}.
...
If  $a=0$,  from Theorem~\ref{Thm(3.1)}, there exists  an infinite dimension center manifold.  If  the parameter $\sigma$ is small enough,  then the above system of \spde{} \eqref{2} has an approximation stochastic slow manifold of $v=\cKo u^2$. The evolution on the center manifold is governed by the following \spde{}, 
\begin{eqnarray*}
