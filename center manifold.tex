...
Random variable $z(\theta_t\omega)$ is the stationary solution of  the Ornstein--Uhlenbeck \sde~(\ref{eq(2.4)}).
...
Note that $z(\theta_s\omega)=-\int_{-\infty}^0\exp(\tau)\omega(\tau+t)\,d\tau+\omega(t)=\int_{-\infty}^t\exp(s-t)dW(s)$.  In fact this is the convolution term~\cite{Rob1}. Then the expectation of stochastic centre manifold
is
...
\frac{\cosh(\pi-\zeta)\cos h z}{\sinh\pi}\,,&z<\zeta\,,\\
