...
\thanks{School of Mathematical Sciences, University of
Adelaide, Adelaide, \textsc{Australia}. \protect\url{mailto:
x. chen@adelaide.edu.au} }
\and {A.~J.
...
\begin{equation}\label{pa}
 u_{t}= u_{xx}+4u+u\circ\dot{W}(t), \quad u(0,t)=u(\pi,t)=0,  \; {\;\; t\geq 0}.
\end{equation}
...
strongly continuous semigroup $S(t):=\exp({At})$ on $H$, which satisfies the
\emph{exponential trichotomy } with exponents $\alpha >\gamma> 0>-\gamma>-\beta$ and bound
$K$.
...
exponential trichotomy  with exponents $\alpha >\gamma> 0>-\gamma>-\beta$, if the following four conditions hold. There exist continuous projection operators $P^c$, $P^u$ and $P^s$ on $H$ such
that
\begin{itemize}
\item[(i)] $\operatorname{id}=P^c+P^s+P^u$,  $P^iP^j=0,$ $ i\neq j$ and $i,j\in\{c,s,u\}$, $\operatorname{id}$ is the identity operator.
\item [(ii)] $P^c \exp (At)=\exp(At)P^c$, $P^u\exp(At)=\exp(At)P^u$, $P^s\exp(At)=\exp(At)P^s$ for $t\geq 0$.
\item[(iii)] Denote the reducing subspaces $H^c:= P^cH$, $H^u:=P^uH$ and  $H^s:=P^sH$. We  call $H^c$,  $H^u$ and $H^s$ the center subspace, the unstable
...
exponential trichotomy.  If $H$ is an infinite dimension space and  the spectrum of  $A$ satisfies $\sigma(A)=\sigma^s\cup\sigma^c\cup\sigma^u$, where $\sigma^s=\{\lambda\in \sigma(A)\mid \operatorname{Re} \lambda \leq-\beta\}$, $\sigma^c=\{\lambda\in \sigma(A)\mid| \operatorname{Re} \lambda| \leq \gamma\}$, $\sigma^u=\{\lambda\in \sigma(A)\mid \operatorname{Re} \lambda \geq \alpha\}$, $\sigma^c$ is finite and $A$ generates a strong continuous semigroup,  then we  have the exponential trichotomy~\cite{Gal, Har}.
\end{remark}

\vskip0.1in We assume the nonlinear term $F$ satisfies $F(0)=0$,  and assume it to be Lipschitz
continuous on $H$, that is,
\[
|F(u_1)-F(u_2)|\le \hbox{{\rm Lip}} F\,|u_1-u_2 |
\]
with the sufficiently small  Lipschitz constant $\hbox{{\rm Lip}} F>0$. If  the  nonlinear term $F$ is locally Lipschitz, let $F^{(R)}(u)=\chi_R(u)F(u)$, where $\chi_R(u)$ is a cut-off function, and then  $F^{(R)}$ is  globally Lipschitz with Lypschitz  constant $R\,\hbox{{\rm Lip}} F$.
...
%The solution of (\ref{eq(2.2)}) can be written as a {\em mild}
%solution:
%\[
...
process. Let $C_0(\mathbb{R},\mathbb{R})$ be continuous functions on $\mathbb{R}$, the associated distribution $\mathbb{P}$ is a Wiener measure defined on the Borel-$\sigma$-algebra $\mathcal{B}(C_0(\mathbb{R},\mathbb{R}))$.  Caraballo et~al.~\cite{Car3} and Duan et al.~\cite{ Duan1} established the following lemma, which is used in the proof of existence of a stochastic  center manifold.
\begin{lemma}\label{lem(2.1)}
\begin{itemize}
\item[(i)]  There exists a $\{\theta_t\}_{t\in\mathbb{R}}$-invariant set
$\Omega\in\mathcal{B}(C_0(\mathbb{R},\mathbb{R}))$ of full measure
with sublinear growth:
\[
\lim_{t\to\pm\infty}\frac{|\omega(t)|}{|t|}=0,\qquad
\omega\in\Omega.
\]\\
\item[(ii)] For $\omega\in\Omega$ the random variable
\[
z(\omega)=-\int_{-\infty}^0e^\tau\omega(\tau)d\tau
\]
exists and generates a unique stationary solution of (\ref{eq(2.4)})
given by
\begin{eqnarray*}
...
\end{eqnarray*}
The mapping $t\to z(\theta_t\omega)$
is continuous.\\
\item[(iii)]  In particular,
\begin{equation*}
%\label{eq401}
...
%$$\int_{-\infty}^t\exp(\tau-t)dW_\tau(\omega). $$



%Using the strong law of large numbers, the function
%$$\tau\to \exp(\tau)\sup\limits_{t\in [t_0-1,t_0+1]}|\omega(\tau+t)|$$
...
We show that  the solution of equation (\ref{eq(2.1)}) define  a random dynamical system.  For any $v\in H,$ and $\omega\in \Omega$,
we introduce the  coordinate transform
\begin{equation}\label{eq(2.7)}
T(v, \omega)=v\exp[{-z(\omega)}]
\end{equation}
and its inverse transform
\begin{equation}\label{eq(2.8)}
T^{-1}(v, \omega)=v\exp[{z(\omega)}].
\end{equation}
...
\[
|G(\omega,u_1)-G(\omega,u_2)|\le \hbox{{\rm Lip}}_uG\,  |u_1-u_2 |,
\]
where $\hbox{{\rm
Lip}}_u G$ denotes the Lipschitz constant of $G(\cdot, u)$ with
respect to $u$.
For  any $\omega\in\Omega$ the function $G$ has the
same global Lipschitz constant  as $F$ by the construsion of $G$. %In contrast to the
%stochastic PDE  (\ref{eq(2.2)}),  no stochastic differential appears in the random PDE  ~\eqref{eq(2.5)}.
Define $$\Psi_A(t,s)=\exp\left[A(t-s)+\int_s^tz(\theta_r\omega)dr\right]$$ as a `state transition operator' for the linear random partial differential equation \eqref{eq(2.5)} in the case
...
Suppose that $u$ is the random dynamical system generated by
(\ref{eq(2.5)}). Then
...
$(t,\omega)\mapsto  \hat u(t, v,\omega)$ is a solution to
(\ref{eq(2.1)}).
\end{lemma}
%\begin{remark}
...
%metric space $(H,d_H)$ is called a {\em random set} if
%\[
...
dynamical system $\Psi(t, \omega, x)$ if
\[
\Psi(t,M(\omega),\omega)\subset M(\theta_t\omega)\;\quad \hbox{for} \;
t\geq 0.
\]
...
\end{align*}
$h^c(0,\omega)=0$, and the tangency condition that  the derivative $Dh^c(0,\omega)=0$,
...
manifold for the stochastic evolution equation
(\ref{eq(2.1)}).
...
space
\begin{align*}
C_\eta=&\left \{\Psi\;\mid\; \Psi\in C(\mathbb{R}, H)\hbox{
and }  \sup_{t\in \mathbb{R}}\exp\left[{-\eta |t| - \int_0^t
z(\theta_r\omega) dr}\right] | \Psi(t)|
 < \infty\right\}
\end{align*}
with the norm
\[\left|\Psi\right|_{C_\eta}=\sup_{t\in \mathbb{R}}\exp\left[{-\eta |t|-\int_0^t z(\theta_r\omega)
dr}\right] |\Psi(t)|.
\]
$C_\eta$ is the set of  slow growth  functions. We know that the functions  are controlled by
...
Different from the proof process on Duan et al~\cite{Duan1,Duan2}, we need analysis the behavior of the  solution on center subspace.
\begin{theorem} \label{Thm(3.1)}    If   $\gamma < \eta<\min\{\beta, \alpha\}$ such that the nonlinearity term is sufficiently small,
\begin{eqnarray}K \;  \hbox{{\rm Lip}}_u G \; \left(\frac{1}{\eta-\gamma}+\frac{1}{\beta-\eta}+\frac{1}{\alpha-\eta}\right)<
...
%exponential trichotomy condition  and $\hbox{{\rm
%Lip}}_u G$ denotes the Lipschitz constant of $G(\cdot, u)$ with
%respect to $u$.
\end{theorem}
...
\begin{proof}
 First we claim that $u_0 \in \
M^c(\omega)$ if and only if  there exists a slow growth function $u(\cdot, u_0, \omega)\in
C_\eta$  with
...
\begin{align}\label{eq(3.3)}\begin{split}
...
\tau}) |u|_{C_\eta} \\
& =K\Psi_{-\beta}(t,0)
\exp[{(\beta+\eta)\tau}] |u|_{C_\eta} \to 0\;\hbox{as }\tau \to -\infty.
\end{align*}
...
\end{align}
Combining  (\ref{eq(3.3)}),  (\ref{eq(3.5)}) and (\ref{eq(3.55)}), we have
(\ref{eq(3.2)}). The converse  follows from a direct computation.


Next we prove  that for any given $v \in H^c$, the centre subspace, the integral
equation (\ref{eq(3.2)}) has a unique solution in the slow growth functions space $C_\eta$.
Let
\begin{align}\label{eq(3.22)}\begin{split}
J^c(u,v):=&\Psi_A(t,0)v+\int_0^t
\Psi_A(t,s)
P^cG(\theta_s\omega,u(s))ds\\&+\int^{+\infty}_{-\infty}\Psi_0(t,s)B(t-s)G(\theta_s\omega,u(s))ds.
\end{split}
\end{align}  $ J^c$ is
well-defined from $C_\eta \times H^c$ to the slow growth functions space $C_\eta$. For each pair of slow growth functions
...
\quad \quad+\int^{+\infty}_{-\infty}\Psi_0(t,s)B(t-s)(G(\theta_s\omega,u)-G(\theta_s\omega,\bar u))ds\bigg|
\Bigg\}\\
&\leq\sup_{t\in \mathbb{R}} \Bigg\{K \hbox{{\rm Lip}}_u G |u-\bar
u|_{C_\eta}\bigg|\int_0^{t} \exp[
{(\gamma-\eta)|t-s|}]ds+\int^t_{-\infty}
\exp[
{(\eta-\beta)(t-s)}]ds\\&  \quad \quad+\int_t^{+\infty}
(-\exp[
{(\alpha-\eta)(t-s)}])ds\bigg |
\Bigg\}\\
&\leq K \hbox{{\rm Lip}}_u
...
% $$K \hbox{{\rm Lip}}_u G\left(\frac{1}{\eta-\gamma}+\frac{1}{\beta-\eta}+\frac{1}{\alpha-\eta}\right)< 1,$$
$ J^c$ is a uniform contraction with respect to the parameter
$v$.  By the uniform contraction mapping principle,
...
)|_{C_\eta}\leq \frac{K}{1-K \hbox{{\rm Lip}}_u
...
$u(t,   \cdot, \omega )$ is Lipschitz from the center subspace $H^c$ to slow growth  functions $C_\eta$.
...
$u(\cdot, v,   \omega)$ is $\mathcal{F}$-measurable. Combining $u(\cdot, v, \omega)$ is  continuous with respect to $H$, we have $u(\cdot , v,   \omega)$ are measurable with respect to $(\cdot, v, \omega)$.
...
%\[
%|h^c(v,\omega) - h^c(\bar v, \omega)|\leq \frac{K^2\hbox{{\rm
%Lip}}_u G}{(\alpha-\eta)\big(1-K \hbox{{\rm Lip}}_u
...
there exists $$u(\cdot, u_0, \omega)~\in~C_\eta$$ with $u(0)=u_0$ and satisfies
(\ref{eq(3.2)}) it follows that $ u_0 \in M^c(\omega)$ if and only
...
\end{equation}
is measurable. Let $H'$ be a countable dense subset of the separable space
$H$.  From  the continuity of $h^c(\cdot,
\omega)$,
...
\begin{theorem} \label{Thm(3.2)} Suppose that $u$ is the random dynamical system generated by random partial differential equation
(\ref{eq(2.5)}), then
$\widetilde {M}^c(\omega)=T^{-1}(\omega,M^c(\omega))$ is a
center manifold of the stochastic partial differential equation
(\ref{eq(2.1)}).
\end{theorem}
\begin{proof}
Denote
$\tilde u(t, \omega, x)$ the solution of (\ref{eq(2.1)}).
...
manifold.
\end{proof}
Note that a locally center manifolds is not unique~\cite{Carr}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Duan et al.'s work~\cite{Duan1} only implies existence of $M^c$ but not the smoothness.
...
\[
K \hbox{{\rm Lip}}_u
G\left(\frac{1}{i\eta-\gamma}+\frac{1}{\beta-i\eta}+\frac{1}{\alpha-i\eta}\right)< 1 \quad\hbox{for
all}\; 1\leq i\leq k,
\]
...
1.
\]
Thus, $J^s(\cdot,v, \omega)$ defined in the proof of Theorem
\ref{Thm(3.1)} is a uniform contraction in $C_{\eta-\eta'}
...
&\quad \quad+\int_{-\infty}^{+\infty} \Psi_0(t,s)
B(t-s)D_uG(\theta_s\omega,u(s,v_0,\omega)) v \,ds.
\end{align*}
From the assumption, we have that
$S$ is a bounded linear operator from center subspace $H^c$ to slow growth functions space
$C_{\eta-\delta}$. Using the same arguments in the proof Theorem  \ref{Thm(3.1)}  that
$J^c$ is a contraction, we have that $T$ is a bounded linear
operator from $C_{\eta-\delta}$ to itself and
\[
\|T\| \leq K \hbox{{\rm Lip}}_u
G\left(\frac{1}{\eta-\delta-\gamma}+\frac{1}{\beta-(\eta-\delta)}+\frac{1}{\alpha-(\eta-\delta)}\right)<
1,
\]
which implies that $\operatorname{id}-T$ is invertible in $C_{\eta-\delta}$.
...
\omega))
=S(v-v_0) + I,\\
%&=S(v-v_0) + o(|v-v_0|), \,\hbox{as}\,v \to v_0.\\
\end{split}
\end{align}
which yields
\begin{align*}
...
\end{align*}
If  $|I|_{C_{\eta-\delta}}=\mathrm{o}(|v-v_0|)$ as $v\to
...
Now we prove that \begin{eqnarray}|I|_{C_{\eta-\delta}} = \mathrm{o}(|v-v_0|)\label{Claim1}\end{eqnarray} as
...
\Bigg \{\bigg |\int_N^t \Psi_A(t,s)
 P^c\Big[G(\theta_s\omega,u(s,v,\omega))-\\ & \quad
G(\theta_s\omega,u(s,v_0,\omega))-D_uG(\theta_s\omega,u(s,v_0,\omega))
(u(s,v,\omega) -u(s,v_0,\omega))\Big]ds\bigg|\Bigg\}\\
\end{align*}
for $t\geq N$.
\begin{align*}
I_1'=&\exp\left[{-(\eta-\delta)|t|-\int_0^t z(\theta_s\omega) ds}\right]
\Bigg\{\bigg|\int^{-N}_t \Psi_A(t,s)
P^c\Big[G(\theta_s\omega,u(s,v,\omega))\\& -
G(\theta_s\omega,u(s,v_0,\omega))-D_uG(\theta_s\omega,u(s,v_0,\omega))
(u(s,v,\omega)-u(s,v_0,\omega))\Big]ds\bigg|\Bigg\}
\end{align*}
...
\Bigg\{\bigg|\int^{\overline N}_t [-\Psi_A(t,s)]
...
\begin{align*}
I_3'=&\exp\left[{-(\eta-\delta)|t|-\int^t_0 z(\theta_s\omega) ds}\right]
\Bigg\{\bigg|\int_{-\overline N}^t \Psi_A(t,s)
...
\Bigg\{\bigg|\int^\infty_{\overline N} [\Psi_A(t,s)]
P^u\Big[G(\theta_s\omega,u(s,v,\omega))\\&-
G(\theta_s\omega,u(s,v_0,\omega))-D_uG(\theta_s\omega,u(s,v_0,\omega))
(u(s,v,\omega) -u(s,v_0,\omega))\Big]ds\bigg|\Bigg\}.
\end{align*}
%For $-\overline{N}\leq t\leq 0$,

\begin{align*}
I_4'=&\exp\left[{-(\eta-\delta)|t|-\int^t_0 z(\theta_s\omega) ds}\right]
\Bigg\{\bigg|\int_{-\infty}^{-\overline N} \Psi_A(t,s)
P^s\Big[G(\theta_s\omega,u(s,v,\omega))\\&-
G(\theta_s\omega,u(s,v_0,\omega))-D_uG(\theta_s\omega,u(s,v_0,\omega))
(u(s,v,\omega) -u(s,v_0,\omega))\Big]ds\bigg|\Bigg\}.
\end{align*}
...
\Bigg\{\bigg|\int^{+\infty}_{t} [\Psi_A(t,s)]
P^u\Big[G(\theta_s\omega,u(s,v,\omega))\\&-
G(\theta_s\omega,u(s,v_0,\omega))-D_uG(\theta_s\omega,u(s,v_0,\omega))
(u(s,v,\omega) -u(s,v_0,\omega))\Big]ds\bigg|\Bigg\}.
\end{align*}
\begin{align*}
I_5'=&\exp\left[{-(\eta-\delta)|t|-\int^t_0 z(\theta_s\omega) ds}\right]
\Bigg\{\bigg|\int_{-\infty}^{t} \Psi_A(t,s)
P^s\Big[G(\theta_s\omega,u(s,v,\omega))\\ & -
G(\theta_s\omega,u(s,v_0,\omega))-D_uG(\theta_s\omega,u(s,v_0,\omega))
(u(s,v,\omega)-u(s,v_0,\omega))\Big]ds\bigg|\Bigg\}.
\end{align*}
...
\begin{align*}
I_1 &\leq 2K\hbox{{\rm Lip}}_uG \int_N^t
\exp[{(\gamma-(\eta-\delta))|t-s|}]\exp({-\delta |s|})
|u(\cdot, v,\omega)-u(\cdot,v_0,\omega)|_{C_{\eta-2\delta}} ds \\
& \leq \frac{2K^2\hbox{{\rm Lip}}_uG \exp({-\delta
N})}{(\eta-\gamma-\delta)\left\{1-K \hbox{{\rm Lip}}_u
...
$N$ so large that
\begin{align*}
 \frac{2K^2\hbox{{\rm Lip}}_uG \exp({-\delta
N})}{(\eta-\gamma-\delta)\left\{1-K \hbox{{\rm Lip}}_u
G\left[\frac{1}{(\eta-2\delta)-\gamma}+\frac{1}{\beta-(\eta-2\delta)}+\frac{1}{\alpha-(\eta-2\delta)}\right]\right\}}
...
\int^1_0\big |
D_uG(\theta_s\omega,\tau u(s,v,\omega)+(1-\tau)u(s,v_0,\omega)) \\
&\quad -D_uG(\theta_s\omega,u(s,v_0,\omega))\big |d\tau\Big\}
|u(\cdot, v,\omega)-u(\cdot,v_0,\omega)|_{C_{\eta-\delta}} ds \\
& \leq \frac{K^2|v-v_0|}{1-K \hbox{{\rm Lip}}_u
G\left[\frac{1}{\eta-\delta-\gamma}+\frac{1}{\beta-(\eta-\delta)}+\frac{1}{\alpha-(\eta-\delta)}\right]}\\&
\quad \int_0^N \exp[{(\gamma+(\eta-\delta))|t-s|}]\Big\{ \int^1_0\big |
D_uG(\theta_s\omega,\tau
u(s,v,\omega)+(1-\tau)u(s,v_0,\omega))\\&
\quad-D_uG(\theta_s\omega,u(s,v_0,\omega))\big | d\tau\Big\}ds.\;
\end{align*}
...
$$
Therefore  $|I|_{C_{\eta-\delta}} = \mathrm{o}(|v-v_0|)$ as $v\to
v_0$.



We now prove that $D_v u(t, v,\omega)$ is
continuous from the center subspace~$H^c$ to slow growth functions space $C_\eta$. For $v,\; v_0~ \in~ H^c$,
using (\ref{eq(4.2)}),
...
\begin{align*} \bar I =&
\int_0^t \Psi_A(t,s)
P^c\big(D_uG(\theta_s\omega,u(s,v,\omega))\\ & \hskip1.5in
-D_uG(\theta_s\omega,u(s,v_0,\omega)
\big)D_v u(s,v_0,\omega)ds\\
& +\int^{+\infty}_{-\infty} \Psi_0(t,s)
B(t-s)\big(D_uG(\theta_s\omega,u(s,v,\omega))\\ & \hskip1.5in
-D_uG(\theta_s\omega,u(s,v_0,\omega)
\big)D_v u(s,v_0,\omega)ds.
\end{align*}
 ...
 C_\eta)}$, we have
\begin{align*}
&|D_v u(\cdot, v,\omega)-D_v u(\cdot,v_0,\omega)|_{L(H^c, C_\eta)}  \leq \frac{|\bar I|_{L(H^c, C_\eta)}}{1- K \hbox{{\rm Lip}}_u
...
$|\bar I|_{L(H^c, C_\eta)} = \mathrm{o}(|v-v_0|)$ as $v \to v_0$. Hence
$D_v u(t, v,\omega)$ is continuous from the center space $H^c$ to bounded linear operators space ${L(H^c,
C_\eta)}$. Therefore, $u(t,v,\omega)$ is $C^1$ from
$H^c$ to $C_\eta$.



Now we show that $u$ is $C^k$ from the center space $H^c$ to slow growth functions space
$C_{k\eta}$ by induction for $k\ge 2$. By the induction
assumption, we know that $u$ is $C^{k-1}$ from $H^c$ to
$C_{(k-1)\eta}$ and the   $(k-1)$st-derivative~$D^{k-1}_v
...
from the induction hypothesis. Thus, using $G$ is
$C^k$, we  verify that $R_{k-1}(\cdot,v,\omega) \in
...
$k=1$  shows that $ D^{k-1}_v u(\cdot, v,\omega)$ is
$C^1$ from $H^c$ to $L^k(H^c,C_{k\eta})$.   This completes the
proof.
\end{proof}
...
\[
K  \hbox{{\rm Lip}}
F \left(\frac{1}{i\eta-\gamma}+\frac{1}{\beta-i\eta}+\frac{1}{\alpha-i\eta}\right)< 1 \quad\hbox{for
all}\; 1\leq i\leq k,
\]
then
...
Since
\[
\widetilde M^c(\omega)=\big\{v+\tilde h^c(v, \omega) \big |\;
v\in H^c\big\},
\]
where $\tilde h^c(v, \omega)=\exp[z(\omega)]h^c(\exp[{-z(\omega)]}v, \omega)$. Since
$h^c(v, \omega)$ is $C^k$ in $v$, $\tilde h^c(v, \omega)$ is
$C^k$ in $v$.
\end{proof}
%\begin{remark}
...
This section proves the stability of the random evolution equation \eqref{eq(2.5)}.  Our main result is Theorem  \ref{th5.3}, which shows that the dynamic behavior of equation  \eqref{eq(2.5)} is exponentially approximated by the   stochastic center manifold. We do not assume $H^s=\emptyset$ or $H^u=\emptyset$.  Instead, we project the solution of equation  \eqref{eq(2.5)} to the stable  and unstable subspaces.  Lemma \ref{lemma9} is a week approach solutions version as it only concerns solutions approach $M^c(\omega)$. It does not assert solutions.



\begin{lemma} \label{lemma9}
Let $M^c(\omega)$ be a  locally stochastic center manifold for stochastic evolution equation \eqref{eq(2.5)}, then there exist positive constants  $U$  and   $\mu$ such that
\begin{eqnarray*}
...
\end{align*}
Since the derivative of  stochastic center manifold is  less than Lipschitz constant $ \hbox{{\rm Lip}}_uG$ and $G$ is Lipschitz continuous,  we have  $|N(\omega, X)|<U_1 |X|$.  We obtain
\begin{eqnarray*}
...
&&+U_1K\int_0^t\exp[{(-\beta+\epsilon) (t-s)}|X(s,u_0,\omega)|ds.\end{eqnarray*}
From  Gronwall's inequality we have the lemma.
\end{proof}
\begin{remark}Similarly we have the conclusion there exist  positive constants  $V$ and  $\nu$ such that \begin{eqnarray*}
|P^uu(t,u_0, \omega)-P^uh^c(P^cu(t,u_0,\omega),\omega)|\leq V\exp({\nu t})|P^uu_0-P^uh^c(v,\omega)|,
\end{eqnarray*}
 $t\leq 0$ small enough.   Note that in the finite dimension case~\cite[Theorem 7.1]{Box}, the conclusions are expressed as the random norm.
\end{remark}
...
Let $M^c(\omega)$ be a  locally stochastic center manifold for stochastic evolution equation~\eqref{eq(2.5)}, then there are positive  random variables $L_1(\omega), L_2(\omega)$ and constants $l_1, l_2$ such that for initial values $u_0\in H$, $v_0\in M^c$, the following
hold:
\begin{eqnarray}
...
\end{align*}
Rewrite the nonlinearity as three steps, so that $N'$ can be bounded.  
\begin{align*}
...
\begin{eqnarray*}
X'(t,u_0,\omega)=\Psi_A(t,0)X'(0,u_0,\omega)+\int_0^t\Psi_A(t,s)N'(X',\omega)ds.\end{eqnarray*}
Since  $G$ is Lipschitz continuous,  there exist positive constants $V_1$, $v_1$ and random variable $V_2(\omega)$ such that
\begin{eqnarray*}
|N'(X',\omega)|\leq V_1\exp(-v_1 t)|(\operatorname{id}-P^c)u_0-h^c(P^cu_0,\omega)|+V_2(\omega)|X'|.
\end{eqnarray*}
...
\begin{align*}
|X'(t,u_0,\omega)|\leq& K \exp[{(\gamma+\epsilon) t}]|X'(0,u_0,\omega)|\\\quad \quad&+K(\gamma+\epsilon+v_1)^{-1} \exp[{(\gamma+\epsilon) t}]V_1|(\operatorname{id}-P^c)u_0-h^c(P^cu_0,\omega)|\\&+V_2(\omega)K\int_0^t\exp[{(\gamma+\epsilon) (t-s)}|X'(s,u_0,\omega)|ds.\end{align*}
...
|P^uu(t,u_0,\omega)-P^u h^c(v(t,v_0,\omega),\omega)|\leq L_3(\omega) \exp({ l_3 t}),
\end{eqnarray}
 $t\leq 0$ small enough.
\end{remark}
\begin{corollary} \label{cor5.4}
Suppose that the zero solution of \eqref{th2} is (asymptotically stable) stable. Then the zero solution of the \eqref{eq(2.5)} is (asymptotically stable) stable. \end{corollary}
\begin{proof}
From  Theorem \ref{th5.3} and Remark \ref{rem1} we conclude the zero   solution of \eqref{th2} is (asymptotically stable) stable on centre, stable, unstable subspaces respectively. We conclude Corollary \ref{cor5.4}.
\end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
where $v\in H^c$.
\begin{theorem}
Suppose that a  $\mathcal{F}$-measurable function $g(0,\omega)=0$, $g_v(0,\omega)=0$ and that $M(g)(v,\omega)=\mathrm{O}(|v|^q)$ for some $q>1$. Then as $v\to 0$,
$$|h^c(v,\omega)-g(v,\omega)|=\mathrm{O}(|v|^q).$$
\end{theorem}
\begin{proof}
...
$$Sw=J^c \left[(w(t,v,\omega)+y(t,v,\omega)\right]-y(t,v,\omega),$$
where the operator $J^c$ is defined by equation \eqref{eq(3.22)}.   The domain $Y$ 
$$Y(\omega)=\left\{w(t,v,\omega)\in C_\eta\mid|w(0,v,\omega)|\leq K(\omega) |v|^q\quad \mbox{for all } v\in H^c\right\}.$$
...
\\  =&\int_{-\infty}^0\Psi_{-A}(0,s)\left[(-A-z(\theta_s\omega))P^sy(0,u_c(s,v,\omega),\omega)+\frac d{ds}P^sy(0,u_c(s,v,\omega),\omega)\right]ds\\&-\int^{\infty}_0\Psi_{-A}(0,s)\left[(-A-z(\theta_s\omega))P^uy(0,u_c(s,v,\omega),\omega)
...
&\quad\quad=\int_{-\infty}^0\Psi_{-A}(0,s)Q^sds+\int^{\infty}_0\Psi_{-A}(0,s)Q^uds,
 \end{align*}
...
\end{align*}
Since $g(0,\omega)=0$, $g_v(0,\omega)=0$ and $g$ is  $\mathcal{F}$-measurable, for a small random variable $\epsilon(\omega) $,  there exists a positive random variable $\delta(\omega) $ such that
\begin{eqnarray*}|g_v(v,\omega)|<\epsilon(\omega)\end{eqnarray*} if $|v|<\delta(\omega) $.
From $M(y)(0,u_c,\omega)=C_1(\omega)|u_c|^q$ for a positive random variable $C_1(\omega)$ and all $v\in H^c$,
$|Q^s|\leq C_2(\omega)|u_c|^q$ and  $|Q^u|\leq C_3(\omega)|u_c|^q$.   Using the Gronwall's  inequality in equation \eqref{eq(6.1)}  we  conclude  $|u_c|\leq C_4(\omega)|v|$.
 \end{proof}

\section{Examples} \label{Sec7}
We present some examples to show our results.   Example \ref{exam1} shows that we  directly use the computer algebra~\cite{Rob} to compute the stochastic center manifolds.  We  also do not need the convolution term.  This is different from the results of Roberts~\cite{Rob1}, which   cutted the stochastic partial differential differential equations to be finite dimensional and then used the finite dimensional stochastic center manifolds theory.
\begin{example}\label{exam1}
Consider the stochastic parabolic equation
\begin{equation}\label{pa}
 u_{t}= u_{xx}+u- a  u^3+  u\circ\dot{W}\;
\end{equation}
with
$$
...
$$\sigma(A)=\{1-n^2\mid n=1,2,\ldots\}. $$
The corresponding complete and orthogonal  eigenfunctions  are $\sin nx$, $n=1,2,\ldots$. The operator $A$  satisfies the exponent trichotomy.  Denote $H^c=\operatorname{span}\{\sin x\}$, $H^s=\operatorname{span}\{\sin nx, n=2,3,\ldots\}$.

The transformed random parabolic equation is
\begin{equation}\label{pa1}
 u_{t}= u_{xx}+u+z(\theta_t\omega)u-a\exp[2z(\theta_t\omega)] u^3. \;
\end{equation}
...
$$M^c(\omega)=\{s\sin x+h^c(s\sin x,\omega)\}=\left \{s \sin x + \sum_{n=2}^\infty c_n(s,\omega)\sin nx\right \},$$
where $c_n(s,\omega)=\mathrm{O}(s^3)$ as $s\to 0$ for $\omega\in \Omega$.
The random dynamical system on the slow manifold $M^c(\omega)$ satisfies
\begin{eqnarray}\label{pa2}
\frac {ds}{dt}\sin x+\sum_{n=2}^\infty c_n'(s,\omega)\sin nx=\sum_{n=2}^\infty[1-n^2+z(\theta_s\omega)] c_n(s,\omega)\sin nx\nonumber\\
+sz(\theta_s\omega)\sin x-{\exp[2z(\theta_s\omega)]} as^3\sin^3 x+\mathrm{O}(s^5). \;
\end{eqnarray}
It follows that
\begin{eqnarray*}
\frac {ds}{dt}=sz(\theta_s\omega)-\frac 3 4 {\exp[2z(\theta_s\omega)]}  as^3+\mathrm{O}(s^5).
\end{eqnarray*}
%So $u=0$ is stable if $a<0$.

We now  calculate the slow manifold $h^c(s\sin x,\omega)$. Assume  $$h^c(s\sin x,\omega)= c_3(s,\omega)\sin 3x+\mathrm{O}(s^5). $$


Since
\begin{eqnarray*}
a\exp[2z(\theta_s\omega)](s\sin x+c_3\sin 3x)^3
 = {as^3}
 \exp[2z(\theta_s\omega)]\sin ^3 x+\mathrm{O}(s^5).\end{eqnarray*}
So $$P^cG(\theta_t\omega,s\sin x+h^c)=\frac 3 4 as^3\exp[2z(\theta_s\omega)]\sin x+\mathrm{O}(s^5), $$ $$P^sG(\theta_t\omega,s\sin x+h^c)=-\frac 1 {4} as^3\exp[2z(\theta_s\omega)]\sin 3x+\mathrm{O}(s^5).$$
From
\begin{align*}
M(h^c)(v,\omega)=&\,h_v^c(v,\omega)[P^cA+z(\theta_t\omega)v+P^cG(\theta_t\omega, v+h^c)]-(\operatorname{id}-P^cA)h^c\\&-z(\theta_t\omega)h^c-P^s G(\theta_t\omega,  v+h^c)-P^uG(\theta_t\omega,  v+h^c),
\end{align*}
and $h^c(v,\omega)=c_3(s,\omega)\sin 3x$, we have $c_3(s,\omega)=\frac 1 {8-z(\theta_t\omega)}\times \frac 1 4 a s^3\exp[2z(\theta_s\omega)]$.
So
 $$h^c(s\sin x,\omega)= \frac 1 {8-z(\theta_s\omega)}\times \frac 1 4 a s^3\exp[2z(\theta_s\omega)]\sin 3x+\mathrm{O}(s^5). $$
\end{example}
\begin{example}
Wang and Duan~\cite{ Wang}  presented a  two dimension center--unstable manifold in a semilinear wave equation.  We show that the   semilinear wave equation has a one dimension center manifold.    %For the deterministic case see  Carr's book~\cite[Example 6.4]{Carr}.

Consider the equation
\begin{equation}\label{HypM}
 u_{tt}+ u_t= \frac 1 4 u_{xx}+u+f(u)+  u\circ\dot{W},\;
\; {\;\; x\in (0,2\pi)},  \quad t\geq 0
\end{equation}
with
$$
 u(0,t)=u(2\pi, t)=0.
...
\Psi_2(t,\omega))\in H$, then $\varphi(t, \Psi(0),\omega)=\Psi(t,\omega)$
...
homeomorphism
$$
T(\omega, (u, v))=(u, v+uz(\omega)),\;\; (u, v)\in{H}
$$
with inverse
$$
T^{-1}(\omega, (u, v))=(u, v-uz(\omega)),\;\; (u,
v)\in{H}.
$$
...
\begin{array}{cc}
    0, &  -{\operatorname{id}}  \\
    \frac 1 4 \frac {\partial^2}{\partial x^2}+\operatorname{id}, &  \operatorname{id}\\
\end{array}
\right),\;\; F(\Psi)= \left(
\begin{array}{cc}
    0   \\
...
with negative real part. By using a new inner product~\cite{Wang},  we  define an equivalent norm on
${H}$ with  $\{(1, \delta_k^\pm)\sin kx \}$ are
 orthogonal. 
...
By Theorem \ref{th5.3}, the asmptotic behavior of the equation \eqref{RHPY} is governed by  the one dimension equation
\begin{eqnarray}
...
\begin{eqnarray}
\dot{s}q_2^+=Z(\theta_t\omega)sq_2^+-s^3q_2^++\mathrm{O}(s^3q_2^+).
\end{eqnarray}
Or
\begin{eqnarray}
\dot{s}=z(\theta_t\omega)s-s^3+\mathrm{O}(s^3).
\end{eqnarray}
...
The stable manifold theorem for stochastic differential equations.
 {\em Ann. Probab.},  \textbf{27},  615--652, 1999.
