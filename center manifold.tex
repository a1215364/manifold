...
and $A$~generates a strong continuous semigroup,  then we  have the exponential trichotomy~\cite{Gal, Har}.
\end{remark}
...
$\Psi(t,M(\omega),\omega)\subset M(\theta_t\omega)$ for 
$t\geq 0$\,.
...
space
\begin{align*}
C_\eta=&\left \{ \Psi\in C(\mathbb{R}, H) \mid  \sup_{t\in \mathbb{R}}\exp\left[{-\eta |t| - \int_0^t
...
\end{theorem}
\begin{proof}
Since \marginpar{What is the meaning of this sentence??  Clarify.}
\begin{equation*}
...
\begin{proof}
From  Theorem~\ref{th5.3} and Remark~\ref{rem1} we conclude that the zero   solution of~\eqref{th2} is (asymptotically stable) stable on centre, stable, unstable subspaces respectively. This establishes Corollary~\ref{cor5.4}.
\end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\maginpar{Clarify $s$ is amplitude and time??}
...
Wang and Duan~\cite{ Wang}  presented a  two dimension center--unstable manifold in a semilinear wave equation.  We show that the   semilinear wave equation has a one dimensional center manifold.    %For the deterministic case see  Carr's book~\cite[Example 6.4]{Carr}.
