...
&=&-\mu\int_{-\infty}^0\exp(\mu\tau)\omega(\tau+t)\,d\tau+\omega(t).
\end{eqnarray*}
...
where $v\in H^c$.
For a  $\mathcal{F}$-measurable function~$w(t,v,\omega)$, define the operator
\begin{equation*}Sw=J^c \left[(w(t,v,\omega)+y(t,v,\omega)\right]-y(t,v,\omega),\end{equation*}
where the operator~$J^c$ is defined by equation~\eqref{eq(3.22)}.
The domain~$Y$
...
For $w\in C_\eta$,  let $u_c(t,v,\omega)$ be the solution of
...
Using Gronwall's  inequality in equation~\eqref{eq(6.1)}  we  conclude  $|u_c|\leq C_4(\omega)|v|$.
\end{proof}
...
Recall that the random variable $z(\theta_t\omega)$ is the stationary solution of  the Ornstein--Uhlenbeck \sde~(\ref{eq(2.4)}) with $\mu=1$.
...
Similar to the equation~\eqref{th2}, the random dynamical system on the slow manifold~$M^c(\omega)$ satisfies
...
+s\sigma z(\theta_t\omega)\sin x\nonumber\\&{}-{\exp[2\sigma z(\theta_t\omega)]} as^3\sin^3 x+\Ord{s^5}.
\label{pa2}
\end{align}
...
\end{equation*}
Since%\marginpar{??where from?? x-ref, justify}
\begin{eqnarray*}
...
\end{align*}
and $h^c(v,\omega)=c_3(t,s,\omega)\sin 3x$\,, we have $c_3(t,s,\omega)=\frac 1 {8-\sigma z(\theta_t\omega)}\times \frac 1 4 a s^3\exp[2\sigma z(\theta_t\omega)]$.
So
...
\begin{eqnarray}
s\sin x+\tilde h^c(s\sin x,\omega)&=&s\sin x+\frac 1{32} a   s^3\sin 3x-\frac 1{256} \sigma z(\theta_t\omega) a s^3\sin 3x\nonumber
\\
&& {}+\Ord{s^5, \sigma^2}, \label{cas1}
\\
...
Denote the noise $\phi(0,\, \{k\})=\int_{-\infty}^t\exp[(-k^2-1)(t-s)]dW(s)$,%\marginpar{??This convolution looks wrong too.  Check??} 
then
\begin{eqnarray}
  u= s\sin x + \frac 1 {32} as^3 \sin 3x -\frac 1 {16} as^3\sigma \phi(0,\{3\})\sin 3x+\Ord{s^5, a^2,\sigma^2}\label{cas2}\\
\mbox{such that } \dot{s}= s\sigma \phi(0,{0}) -\frac 3 4 as^3 +  \Ord{s^5, a^3,\sigma^2}.  \nonumber
 \end{eqnarray}
Since
$z(\theta_t\omega)=\phi(0,\, \{0\})$,  this implies  that  both the stochastic slow manifolds \eqref{cas1} and \eqref{cas2}  have a similar approach.
...
Let $\Psi=rq_1^++sq_2^++y$\,, $y\in P^s H$\,.
...
we know that  the asymptotic behavior of the random system~\eqref{RHPY} is governed by  the one dimensional equation%\marginpar{??why one-D, it looks like the emergent dynamics would be two-D to me??  where is $r$?}
\begin{eqnarray}
...
If  $a=0$,  from Theorem~\ref{Thm(3.1)}, there exists  an infinite dimension center manifold.
If  the parameter $\sigma$ is small enough,  then the above system of \spde{} \eqref{2} has an approximation stochastic slow manifold of $v=\cKo u^2$.
The evolution on the center manifold is governed by the following \spde{},
\begin{eqnarray*}
\D tu&=&au+\sigma z(\theta_t\omega)u-\exp(\sigma z(\theta_t\omega))u\cKo u^2.
\end{eqnarray*}
