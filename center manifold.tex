...
The random variable $z(\theta_t\omega)$ is the stationary solution of  the Ornstein--Uhlenbeck \sde~(\ref{eq(2.4)}) and the transform $u=u^*\exp(\sigma z(\theta_t\omega))$. \marginpar{??from (38)?? you must cross-reference explicitly}
...
Select parameter~$a$ small enough such the conditions of  Theorem~\ref{Thm(3.1)} is satisfied.  Then we have\marginpar{??clarify "we have": does it mean "exists"? or something else? does theorem~5 justify the statement? or not?  clarify. In general, do not use "we have"} the slow (center) manifold
...
The random dynamical system on the slow manifold~$M^c(\omega)$ satisfies\marginpar{??justify, cross-reference explicitly??}
...
\end{eqnarray}
It follows that\marginpar{??justify??}
\begin{eqnarray*}
\frac {ds}{dt}=s\sigma z(\theta_t\omega)-\frac 3 4 {\exp[2\sigma z(\theta_t\omega)]}  as^3+\Ord{s^5}.
\end{eqnarray*}
%So $u=0$ is stable if $a<0$.
We now  calculate the slow manifold~$h^c(s\sin x,\omega)$. Assume \marginpar{??what evidence is there that this form is good??} 
...
$z(\omega)$~is the stationary solution of~(\ref{eq(2.4)}). Then we have\marginpar{??replace "we have" by explicit cross-reference: presumably you mean (39)--(40).??}
...
satisfies the exponent trichotomy~\ref{??} with $\alpha=\frac 1 2+\epsilon$\,, $\gamma=\frac 1 4 +\epsilon$
...
Let $\Psi=rq_1^++sq_2^++y$\,, $y\in P^s H$\,.  From the random system~\eqref{RHPY}, \marginpar{why?? justify??}
\begin{eqnarray}
...
By Theorem~\ref{th5.3},\marginpar{??why does it apply??} the asymptotic behavior of the random system~\eqref{RHPY} is governed by  the one dimensional equation\marginpar{??why one-D, it looks like the emergent dynamics would be two-D to me??  where is $r$?}
\begin{eqnarray}
...
Transform the \spde{}s~\eqref{1} to the following system of \spde{}s with random coefficient:
