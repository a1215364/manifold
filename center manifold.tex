...
u=T(u^*, \omega)=u^*\exp[{-z(\omega)}]
\end{equation}
and its inverse transform
\begin{equation}\label{eq(2.8)}
T^{-1}(u^*, \omega)=u^*\exp[{z(\omega)}].
\end{equation}
...
We present some examples to show our results.   Example~\ref{exam1} shows that we  directly use the computer algebra~\cite{Rob} to compute the stochastic center manifolds.  We  also do not need the convolution term.  This is different from the results of Roberts~\cite{Rob1}, which  assume the existence of theory of center manifolds in the stochastic partial differential equations.
