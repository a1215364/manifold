...
\[
\Psi(t,M(\omega),\omega)\subset M(\theta_t\omega)\quad \text{for }
t\geq 0.
\]
...
space
\begin{align*}
C_\eta=&\left \{\Psi\;\mid\; \Psi\in C(\mathbb{R}, H)\text{
...
\begin{proof}
 First we claim that $u_0 \in M^c(\omega)$ if and only if  there exists a slow growth function $u(\cdot, u_0, \omega)\in
C_\eta$  with
...
\tau} )|u|_{C_\eta} \\
& = K\Psi_\alpha(t,0)
\exp[{-(\alpha-\eta)\tau}] |u|_{C_\eta} \to 0\;\text{as }\tau \to +\infty.\\
\end{align*}
For  $t > \tau $
\begin{align*}
...
\tau}) |u|_{C_\eta} \\
& =K\Psi_{-\beta}(t,0)
\exp[{(\beta+\eta)\tau}] |u|_{C_\eta} \to 0\;\text{as }\tau \to -\infty.
\end{align*}
...
there exists \begin{equation*}u(\cdot, u_0, \omega)~\in~C_\eta\end{equation*} with $u(0)=u_0$ and satisfies~(\ref{eq(3.2)}) it follows that $ u_0 \in M^c(\omega)$ if and only
...
\begin{theorem} \label{Thm(3.2)} Suppose that $u$ is the random dynamical system generated by random partial differential equation~(\ref{eq(2.5)}), then
...
\end{align*}
From the assumption, 
$S$ is a bounded linear operator from center subspace~$H^c$ to slow growth functions space~$C_{\eta-\delta}$. Using the same arguments in the proof Theorem~\ref{Thm(3.1)}  that
...
Now we show that~$u$ is~$C^k$ from the center space~$H^c$ to slow growth functions space~$C_{k\eta}$ by induction for $k\ge 2$. By the induction
...
$h^c(v, \omega)$ is~$C^k$ in~$v$, $\tilde h^c(v, \omega)$ is~$C^k$ in~$v$.
\end{proof}
%\begin{remark}
...
Let~$M^c(\omega)$ be a  locally stochastic center manifold for stochastic evolution equation~\eqref{eq(2.5)}, then there are positive  random variables $L_1(\omega)$~and~$L_2(\omega)$ and constants $l_1$~and~$l_2$ such that for initial values $u_0\in H$ and $v_0\in M^c$, the following
hold:
\begin{eqnarray}
...
\begin{eqnarray*}
X'(t,u_0,\omega)=\Psi_A(t,0)X'(0,u_0,\omega)+\int_0^t\Psi_A(t,s)N'(X',\omega)ds.\end{eqnarray*}
Since  $G$ is Lipschitz continuous,  there exist positive constants $V_1$~and~$v_1$ and random variable~$V_2(\omega)$ such that
\begin{eqnarray*}
...
+sz(\theta_s\omega)\sin x-{\exp[2z(\theta_s\omega)]} as^3\sin^3 x+\Ord{s^5}. \;
\end{eqnarray}
It follows that
\begin{eqnarray*}
\frac {ds}{dt}=sz(\theta_s\omega)-\frac 3 4 {\exp[2z(\theta_s\omega)]}  as^3+\Ord{s^5}.
\end{eqnarray*}
%So $u=0$ is stable if $a<0$.

We now  calculate the slow manifold~$h^c(s\sin x,\omega)$. Assume  \begin{equation*}h^c(s\sin x,\omega)= c_3(s,\omega)\sin 3x+\Ord{s^5}. \end{equation*}
...
a\exp[2z(\theta_s\omega)](s\sin x+c_3\sin 3x)^3
 = {as^3}
 \exp[2z(\theta_s\omega)]\sin ^3 x+\Ord{s^5}.\end{eqnarray*}
So \begin{equation*}P^cG(\theta_t\omega,s\sin x+h^c)=\frac 3 4 as^3\exp[2z(\theta_s\omega)]\sin x+\Ord{s^5}, \end{equation*} \begin{equation*}P^sG(\theta_t\omega,s\sin x+h^c)=-\frac 1 {4} as^3\exp[2z(\theta_s\omega)]\sin 3x+\Ord{s^5}.\end{equation*}
From
\begin{align*}
...
So
 \begin{equation*}h^c(s\sin x,\omega)= \frac 1 {8-z(\theta_s\omega)}\times \frac 1 4 a s^3\exp[2z(\theta_s\omega)]\sin 3x+\Ord{s^5}. \end{equation*}
\end{example}
\begin{example}
...
Define \begin{equation*} \mathcal{A}=\left(
\begin{array}{cc}
    0, &  -{\operatorname{id}}  \\
...
\end{array}
\right),\end{equation*}
\begin{equation*} Z(\theta_t\omega)=\left(%
\begin{array}{cc}
       z(\theta_t\omega), & 0 \\
